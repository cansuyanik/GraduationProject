...
def create_weight_matrix(word2idx, embeddings_index, n_words, vector_dimension=300):
...
def plot_the_model(model, model_name):
...
embeddings_index = loadEmbeddings(vectorFile)
embedding_matrix = create_weight_matrix(word2idx, embeddings_index, n_words)

model_name1 = 'pre-trained-rnn-bi211'
model_name2 = 'pre-trained-rnn-3layer222'
model_name3 = 'pre-trained-rnn-dif_parameters233'
    
    
model = createModel(MAX_LEN, n_words+1, n_tags, embedding_matrix=embedding_matrix, bi_direc=True)
...
model4 = createModel(MAX_LEN, n_words+1, n_tags, embedding_matrix=embedding_matrix, lstm_cells=256, lstm_layers=5, bi_direc=True)

print(model4.summary()) 
from IPython.display import Image
plot_model(model3, to_file=f'{model_dir}{model_name3}.png', show_shapes=True)
...
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)

exampleSentence = "sende mi buradasın"
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)

exampleSentence = "benmi geleceğim"
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)

exampleSentence = "buraya ev geldi"
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)

exampleSentence = "okul bıraktı"
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)


exampleSentence = "bunu sende yapmalısın"
predict(model2, words, tags, exampleSentence, word2idx, MAX_LEN, n_words)
...
file1 = open(file_name5, "r", encoding="utf8")
Lines = file1.readlines() 
for lines in Lines: 
    lines = lines.lower()
    lines = re.sub(r'[^\w\s]','',lines)
    predict(model3, words, tags, lines, word2idx, MAX_LEN, n_words, save=True, outputFile= outputFile)
